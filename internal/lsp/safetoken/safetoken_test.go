...
func TestTokenOffset(t *testing.T) {
	...
	}, "go/token", "github.com/cowpaths/golang-x-tools/internal/lsp/...", "github.com/cowpaths/golang-x-tools/gopls/...")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, pkg := range pkgs {
		...
		case "go/token":
			tokenPkg = pkg
		case "github.com/cowpaths/golang-x-tools/internal/lsp/safetoken":
			safePkg = pkg
		}
	}
	...
	if safePkg == nil {
		t.Fatal("missing package github.com/cowpaths/golang-x-tools/internal/lsp/safetoken")
	}
	...
	for _, pkg := range pkgs {
		...
		for ident, obj := range pkg.TypesInfo.Uses {
			...
			t.Errorf(`%s: Unexpected use of (*go/token.File).Offset. Please use github.com/cowpaths/golang-x-tools/internal/lsp/safetoken.Offset instead.`, fset.Position(ident.Pos()))
		}
	}
}
