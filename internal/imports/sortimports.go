...
func sortImports(localPrefix string, tokFile *token.File, f *ast.File) {
	for i, d := range f.Decls {
		...
		for j, s := range d.Specs {
			if j > 0 {
				for tokFile.Line(s.Pos()) > 1+tokFile.Line(d.Specs[j-1].End()) {
					tokFile.MergeLine(tokFile.Position(d.Specs[j-1].End()).Line)
				}
			}
		}
...
// Taken from github.com/cowpaths/golang-x-tools/ast/astutil.
