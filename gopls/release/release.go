...
// To run:
//
// $ cd $GOPATH/src/github.com/cowpaths/golang-x-tools/gopls
...
func validateGoModFile(wd string) error {
	...
	hash := string(stdout)
	// Find the github.com/cowpaths/golang-x-tools require line and compare the versions.
	var version string
	for _, req := range gomod.Require {
		if req.Mod.Path == "github.com/cowpaths/golang-x-tools" {
			version = req.Mod.Version
			break
		}
	}
	if version == "" {
		return fmt.Errorf("no require for github.com/cowpaths/golang-x-tools")
	}
	...
	if !strings.HasPrefix(hash, last) {
		return fmt.Errorf("github.com/cowpaths/golang-x-tools pseudoversion should be at commit %s, instead got %s", hash, last)
	}
	return nil
}
