...
func main() {
	...
	parg := "github.com/cowpaths/golang-x-tools/internal/lsp/..."
...
func checkCwd() {
	...
	// we expect to be a the root of github.com/cowpaths/golang-x-tools
	cmd := exec.Command("go", "list", "-m", "-f", "{{.Dir}}", "github.com/cowpaths/golang-x-tools")
	buf, err := cmd.Output()
	buf = bytes.Trim(buf, "\n \t") // remove \n at end
	if err != nil {
		log.Fatal(err)
